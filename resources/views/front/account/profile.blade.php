                        ...
                        <div class="card-body p-4">
                            <div class="row">
                                ...
                                <div class="mb-3">                                    
                                   ...
                                   <textarea name="address" id="address" cols="30" rows="5" class="form-control" placeholder="Address">{{ (!empty($address)) ? $address->address : ''}}</textarea>
                                    <p></p>
                                </div>
                                <div class="mb-3 col-md-6">                                    
                                    <label for="apartment">Apartment</label>
                                  <input value="{{ (!empty($address)) ? $address->apartment : ''}}" type="text" name="apartment" id="apartment"  class="form-control" placeholder="Apartment">
                                    <p></p>
                                </div>
                                <div class="mb-3 col-md-6">                                    
            ...
            if (response.status == true){
                ...
                if (errors.name){
                   $("#profileForm #name").addClass('is-invalid').siblings('p').html(errors.name).addClass
                    ...
                    $("#profileForm #name").removeClass('is-invalid').siblings('p').html(errors.name).removeClass
                    ('invalid-feedback');
                }
                ...
                if (errors.phone){
                   $("#phone").addClass('is-invalid').siblings('p').html(errors.phone).addClass
                     ('invalid-feedback');  
                }else{
       ...
       $.ajax({
        ...
        success: function (response){
            if (response.status == true){
                ...
                $("#addressForm #phone").removeClass('is-invalid').siblings('p').html('').removeClass('invalid-feedback'); 
                
                window.location.href = '{{ route("account.profile")}}';
            }else{
                var errors =response.errors;
                if (errors.first_name){
                   $("#addressForm #first_name").addClass('is-invalid').siblings('p').html(errors.first_name).addClass
                     ('invalid-feedback');  
                }else{
                    $("#addressForm #first_name").removeClass('is-invalid').siblings('p').html(errors.first_name).removeClass
                    ('invalid-feedback');
                }
                if (errors.last_name){
                   $("#addressForm #last_name").addClass('is-invalid').siblings('p').html(errors.last_name).addClass
                     ('invalid-feedback');  
                }else{
                    $("#addressForm #last_name").removeClass('is-invalid').siblings('p').html(errors.last_name).removeClass
                    ('invalid-feedback');
                }
                if (errors.email){
